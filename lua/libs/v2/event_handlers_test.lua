function pre__TestCaseEventHandler1(i1, i2, p3)
    LE.LOGGER:LogInfo("pre__TestCaseEventHandler1")
    local v_int = LE.MEMORY:ReadInt(p3)
    LE.LOGGER:LogInfo(string.format("%d %d %d (0x%X)", i1, i2, v_int, p3))
    
    LE.MEMORY:WriteInt(p3, 9999)
end

function pre__TestCaseEventHandler2(i1, i2, p3)
    LE.LOGGER:LogInfo("pre__TestCaseEventHandler2")
end

function post__TestCaseEventHandler1(i1, i2, psomeint, result)
    LE.LOGGER:LogInfo("post__TestCaseEventHandler")
    
    local v_int = LE.MEMORY:ReadInt(result)
    LE.LOGGER:LogInfo(string.format("%d %d result %d (0x%X)", i1, i2, v_int, result))
...
-- LE.LOGGER:LogInfo("GetEventHandlers")
